"use server";

import { revalidatePath } from "next/cache";
import { apiDelete, apiPostFormData } from "@/lib/api-client";
import { requireAdminAccess } from "@/features/admin/session/require-admin";
import { getAdminAccessToken } from "@/features/admin/session/session";

export type CategoryProcessImage = { id: string; url: string; altText: string | null } | null;
type UploadResult = { image?: CategoryProcessImage; message: string };
type MutationResult = { ok: boolean; message: string };

async function token(): Promise<string | null> {
  const access = await requireAdminAccess("categories");
  return access.state === "authorized" ? getAdminAccessToken() : null;
}

/**
 * Uploads always replace: the backend (`CategoriesAdminService.uploadImage`) deletes any
 * existing row for this category before writing the new one, since a Category takes exactly one
 * process photograph, never a gallery.
 */
export async function uploadCategoryImage(
  categoryId: string,
  form: FormData,
): Promise<UploadResult> {
  const accessToken = await token();
  if (!accessToken) return { message: "Your editing session is unavailable." };
  const image = form.get("image");
  const altText = form.get("altText");
  if (!(image instanceof File) || typeof altText !== "string" || !altText.trim())
    return { message: "Choose an image and enter descriptive alt text." };
  const body = new FormData();
  body.set("image", image);
  body.set("altText", altText.trim());
  const result = await apiPostFormData<{ id: string; url: string; altText: string | null }>(
    `/admin/catalog/categories/${encodeURIComponent(categoryId)}/image`,
    body,
    { accessToken },
  );
  if (!result.ok) return { message: "The image could not be uploaded." };
  revalidatePath("/admin/catalog/categories");
  revalidatePath("/en/products", "layout");
  return { image: result.data, message: "Image uploaded." };
}

export async function removeCategoryImage(
  categoryId: string,
  imageId: string,
): Promise<MutationResult> {
  const accessToken = await token();
  if (!accessToken) return { ok: false, message: "Your editing session is unavailable." };
  const result = await apiDelete<{ deleted: true }>(
    `/admin/catalog/categories/${encodeURIComponent(categoryId)}/image/${encodeURIComponent(imageId)}`,
    { accessToken },
  );
  revalidatePath("/admin/catalog/categories");
  revalidatePath("/en/products", "layout");
  return result.ok
    ? { ok: true, message: "Image removed." }
    : { ok: false, message: "The image could not be removed." };
}
