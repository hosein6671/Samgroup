...
**Site-wide consumption is not implemented.** §4's `generateMetadata` mapping exists on the `cms-proof` demonstration route and, since 17 August 2026, on the canonical `/{locale}/privacy-policy` route — as proof that CMS-authored SEO reaches Next's Metadata API through NestJS, not as a site-wide implementation. There is still no `metadataBase`, no `hreflang` rendering, no JSON-LD renderer, and no `sitemap.ts`/`robots.ts`. `robots` is deliberately **not** mapped on either route: Next resolves metadata by nearest value, so emitting it would override the `noindex, nofollow` the `[locale]` layout sets for the whole tree — on the proof route that would publish non-authoritative demo content, and on a legal route it would lift a blanket directive the platform has not yet decided to lift. Honouring an editor's per-page `noindex` belongs to the launch gate that removes the blanket one.

**One addition on the canonical route:** `canonicalUrl` is emitted as `alternates.canonical` when — and only when — an editor set one. That is the field's contracted meaning (§2: an override), and the fallback "the entity's own resolved URL" is deliberately **not** applied, because composing it needs a public origin no route may choose for itself. `alternates`/`hreflang` are unmapped for the same reason. Both arrive with `metadataBase`.
