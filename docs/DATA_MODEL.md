...
- **That invariant's enforcement mechanism is now decided — [ADR-011](./ADR/ADR-011-products-slug-namespace-enforcement.md) — and is still not built.** ADR-010 §6 requires enforcement before the first real `PRODUCT` write path, before Product reference data is populated, and before the first `CATEGORY` or `PRODUCT` translated-slug row; ADR-011 closes the mechanism deferral it left open. **The accepted model is a shared claim registry, maintained by the database:**
  ...
  **Nothing of this exists yet: no table, no index, no trigger, no migration, and no change to `prisma/schema.prisma`.** ADR-011 is an accepted decision, and its installation — which must be atomic, with a generic backfill of every existing base and translated slug rather than a DEV-specific one — is the next gate. Deferral of that gate rests on a scoped fact: in local DEV `sam_platform` as of 14 August 2026, `products` holds no rows and no `slug` translation row exists for any `CATEGORY` or `PRODUCT`. That is a statement about one development database on one date, not a property of the model.
