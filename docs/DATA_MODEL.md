...
- **The products slug namespace is shared, and its uniqueness invariant spans two entities and every locale — [ADR-010](./ADR/ADR-010-products-slug-namespace-and-collision-policy.md).** `CATEGORY.slug` (Product Family) and `PRODUCT.slug` (Product Detail) both compose the same public URL shape, `/{locale}/products/{slug}`, by decision. The schema does **not** currently express that: `CATEGORY.slug` and `PRODUCT.slug` carry independent unique keys on two different tables, and `CONTENT_TRANSLATION` is unique on `(entityType, entityId, locale, field)` — never on the translated **value** — so two entities may hold the same translated slug and `findEntityIdBySlug`'s `findFirst` would resolve the ambiguity arbitrarily. The required invariant is stated symmetrically: the namespace is the **union** of base `CATEGORY` slugs, base `PRODUCT` slugs, translated `CATEGORY` slug values and translated `PRODUCT` slug values, and **any new slug value introduced by either entity, in any locale, must not already exist elsewhere in that union where it would create ambiguity** — a colliding `CATEGORY` is exactly as invalid as a colliding `PRODUCT`. `finder`, `segments` and `types` are reserved against all four. **Colliding data is invalid**, not merely deprioritised; Product Family precedence is the runtime safety rule behind it, not a substitute for it.
- **That invariant needs durable enforcement, and it is required before the relevant writes — not yet.** ADR-010 §6 requires enforcement to exist before the first real `PRODUCT` write path, before Product reference data is populated, and before the first `CATEGORY` or `PRODUCT` translated-slug row. **The mechanism is deliberately undecided** — a shared slug registry table, a PostgreSQL constraint trigger and application/admin validation are all recorded as candidates and none is selected here; note only that application validation alone is not equivalent to a database-enforced invariant, because seeds, imports and direct SQL bypass it. **No column, constraint, index or table changes as a result of ADR-010.** Deferral is possible because, in local DEV `sam_platform` as of 13 August 2026, `products` holds no rows and no `slug` translation row exists for any `CATEGORY` or `PRODUCT`; that is a scoped statement about one development database on one date, not a property of the model.
...
- `INQUIRY.relatedProductId` is nullable and only meaningful for `Sample Request` (and optionally `Product Inquiry`) submissions — it records which product page the CTA was clicked from, replacing the `productId` the old `SAMPLE_REQUEST` carried.
