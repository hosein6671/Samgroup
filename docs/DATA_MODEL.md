...
- **`INQUIRY.status` and `CUSTOM_FORMULATION_REQUEST.status` now hold one of three values — `new`, `in_progress`, `closed`** ([ADR-013](./ADR/ADR-013-lead-assignment-and-status-workflow.md)), constrained by a database CHECK added in migration `20260820120000_add_lead_workflow`. `new` keeps its exact previous value and meaning, so no row moved. The graph is `new→in_progress`, `new→closed`, `in_progress→closed` and `closed→in_progress`; `closed→new` and every self-transition are refused by the Forms module, because a CHECK cannot see the previous value. **`assigned` is deliberately not a member** — ownership is `assignedToId`, and a status repeating it would be a second source of truth. `STATUS_HISTORY` now carries `changedByEmailSnapshot`, and the new `LEAD_ASSIGNMENT_HISTORY` records ownership changes with both ends snapshotted, so an audit row still names people after a `User` is deleted. The paragraph below is the superseded record of the single-value state:

  - **[SUPERSEDED 20 August 2026] `INQUIRY.status` and `CUSTOM_FORMULATION_REQUEST.status` hold exactly one value: `new` — ratified, with its meaning frozen narrowly.** `new` is the **initial ingestion state only**: the submission was accepted and written, and nobody has looked at it. It does **not** define a workflow or a lifecycle, **no transition is authorized**, and no second value and no ordering exist. Both columns are `String` because, as the schema states, the business lifecycle is not defined; the status vocabulary and the Admin/RBAC workflow that would operate it remain deferred, and that gate is free to rename this value while it is still the only one in the table. The value is **server-owned**: it lives in one constant in `apps/api` and neither DTO accepts a `status` field, so a client can neither submit nor override it. No `STATUS_HISTORY` row is written, because no status ever changes.

- **`INQUIRY.privacyPolicyVersion` and `CUSTOM_FORMULATION_REQUEST.privacyPolicyVersion` are consent evidence — nullable `text`, server-owned, written once.** They record **which** Privacy Policy revision a submitter agreed to; `createdAt` already records when. Added 18 August 2026 by migration `20260818120000_add_consent_privacy_policy_version`, additively and on these two entities only. **The value comes from `apps/api`** — the constant `ACTIVE_PRIVACY_POLICY_REVISION` in the Forms module — never from the request body (neither DTO declares the field, so sending it is a 400) and never from a lookup in `sam_cms`: a stored literal cannot be silently rewritten by a later CMS edit, and a form submission must not depend on CMS availability. **Not a foreign key and not a Payload row or revision id** — ADR-002 keeps the two databases independent, and `Pages.lastUpdatedDate` is an editor-set display field, not an audit trail ([SECURITY.md](./SECURITY.md#personal-data-retention)). **NULL means no versioned policy was in force**, which is the truth of every row written while no approved Privacy Policy exists and the consent labels link to nothing; no value was invented and nothing was backfilled. **The value is immutable after insertion, enforced by PostgreSQL** — a `BEFORE UPDATE` trigger on both tables (migration `20260818140000_privacy_policy_version_immutable`) rejects any change where the column `IS DISTINCT FROM` its stored value, including `NULL` to a revision, so a historical consent can never be retro-fitted with a policy that did not exist. Every other column on both entities stays normally updateable, which the eventual lead workflow needs.
  Both tables were empty in local DEV when the column was added. The other four submission entities are unchanged — they have no live endpoint yet, and widening the change beyond the two implemented write paths was not in scope.
