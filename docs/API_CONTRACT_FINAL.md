...
| Method | Path                               | Auth | Purpose                                                                                                                                                                                                                                                     |
| ------ | ---------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/content/globals/:name`           | P    | One Payload Global: `home`, `about-us` (**built** — §2.4b), `products-landing`, `customized-solutions` (**built** — §2.4b), `export-logistics`, `quality-certifications` (**built** — §2.4b), `contact-us` (**contact channels built** — §2.4b), `faq-page` |
...
The Globals path now answers to **four names** — `about-us`, `customized-solutions`, `quality-certifications`, and the contact-channel slice of `contact-us`. Every other name in the §2.4 table is still a **404 decided in the controller, before any CMS request**.
...
#### 2.4b `GET /content/globals/:name` — company Globals

**Four names answer: `about-us`, `customized-solutions`, `quality-certifications`, and `contact-us`.** Contact Us currently projects only independently confirmed contact channels: main/sales phone, general/sales email, WhatsApp, LinkedIn, Instagram, Telegram, and localized address. All fields are nullable and an empty published document is `available: false`; Payload metadata never crosses the projection. The other four names that §2.4's table lists are separate gates, and an
...
## ADR-024 initial audit read contract

Product editorial continuation: `content.applications` and `content.features` accept up to 20 rows each, title 1–160 characters and description 1–2000. `content.faq` accepts up to 20 question/answer rows, question 1–300 and answer 1–4000. All row text must contain a non-whitespace character; unknown properties and null arrays are rejected. Omitted arrays preserve current draft values for old clients. Explicit `[]` removes that section on publication. `GET /products/:slug` adds optional `editorial` with only these three published arrays for `locale=en`; never draft content/revision/SEO. Other locales and catalogue lists are unchanged. These are ordinary product editorial fields, not approved technical facts or a way to approve claims.

Product editorial slice: Admin and Content Manager may GET `/admin/products` (page 1–100000, q up to 100 characters, category UUID optional; 20 rows, name/id ordering), GET `/admin/products/:id` and PATCH `/admin/products/:id`. PATCH requires revision integer, action `save-draft` or `publish`, and content `{name, description, seo}`. Name is 1–200 characters, description up to 10000. SEO accepts optional bounded text fields metaTitle, metaDescription, canonicalUrl, ogTitle, ogDescription, twitterTitle, twitterDescription, twitterCardType (summary/summary_large_image), robotsIndex/robotsFollow booleans and keywords (up to 30 strings, 100 characters each). No free structured data, media URLs or technical fields. Unknown fields are validation errors. Slug stays immutable. Conflicts return 409; draft writes do not change public reads. Publication updates existing Product/SeoMeta and audit atomically. New products, unpublication, media and technical writes are separate slices, not silently enabled by this route.

Additional Admin-only read: `GET /admin/content-events`, with `page` (1–100000; 50 rows), optional actor UUID, `event` (`content.save-draft` or `content.publish`) and inclusive ISO `from`/`to`. Returns the standard data/meta list envelope, projecting durable CMS receipts to id, occurredAt, actorId, subjectId (page key), event, outcome (`success`) and httpStatus (`null`). No request hash, body, credential or media locator is exposed. The unified `/admin/audit` page selects security or content activity; each source has independent pagination. CMS outages are unavailable responses, never an empty history. Nest authorizes Admin and reads through the Content service; Payload REST collection access stays closed. No replicated audit database or false cross-database atomicity is introduced.

`GET /admin/audit-events?page=1` requires Admin JWT, returns no-store `{data:[{id,occurredAt,actorId,subjectId,event,outcome,httpStatus}],meta:{total,page,limit:50}}`. Page is an integer 1–100000. Unknown query fields are rejected. No write/delete audit endpoint. Optional filters: actorId (UUID), event (known event name), outcome (success/failure), from/to (ISO timestamps, inclusive; from must not exceed to). Filters apply equally to rows and total. User mutations and their success events commit in the same transaction; content activity logging remains outstanding.

### ADR-024 user management writes

Admin-only `POST /admin/users` accepts `{email,password,role}` (email max 254, password 12–1024, existing lowercase role vocabulary). `GET /admin/users/:id` returns `{id,email,role,status,adminRevision}`. `PATCH /admin/users/:id` accepts `{role,status,revision}`; lowercase status active/disabled. Unknown fields are rejected. Duplicate email, stale revision, concurrent modification and self-account changes return 409. Mutations run at serializable isolation with an in-transaction actor check and same-transaction audit insert. Password hashes are never returned. Disable retains ADR-012 database-trigger revocation. No account-delete/reset-password endpoint is implemented.

### ADR-024 company content editor (implementation in progress)

`GET /admin/content/:key` and `PATCH /admin/content/:key` admit Admin and Content Manager only. The initial key allowlist is `about-us`, `customized-solutions`, `quality-certifications`, `contact-us`; locale is English for this rollout. GET exposes editable fields plus revision, never internal credentials. PATCH accepts operationId UUID, revision, action (`save-draft` or `publish`) and the exact editable fields for that resource. Unknown fields and immutable identity fields are rejected. Errors: invalid data 400; unauthenticated 401; forbidden 403; stale revision 409; unavailable CMS 503. Saving drafts preserves published content. The CMS owns mutation audit and operation recovery atomically; Nest must not claim cross-database atomicity.

### ADR-024 category narrative endpoints — 10 September 2026

The existing `/admin/content/:key` read/PATCH contract also accepts `category-` followed by one frozen Product Family slug. Authorization, revision, operation UUID, audit transaction and draft/publish semantics match company editing. Content is limited to the narrative fields and bounds documented in PAYLOAD_CONTENT_ARCHITECTURE's category narrative slice; unknown fields are rejected. A never-saved category returns revision `initial` and empty fields without creating a document. Concurrent first creation is protected by the unique categoryKey and serializable transaction.

`GET /content/product-categories/:key?locale=en` returns `{ available, fields }` inside the standard envelope. Only published content is projected; internal IDs, drafts, receipts and credentials are omitted. Known non-English locales return available false during this English-only rollout. Unknown keys are 400, CMS failures 503, successful absence available false. Public route falls back to existing category copy on absence/unavailability; no publication modifies catalog identity, taxonomy, specifications or SEO.

### ADR-024 category SEO slice — 10 September 2026

The category editor's `seo` group is the single editing location for category page SEO: optional metaTitle/ogTitle/twitterTitle (200 characters), metaDescription/ogDescription/twitterDescription (1000), canonicalUrl (2048, absolute HTTPS, no credentials or fragment), twitterCardType (summary/summary_large_image), robotsIndex/robotsFollow (boolean) and keywords (up to 30 nonblank strings, 100 characters each). Empty canonical clears its override. Unknown nested fields, wrong types, unsafe URLs and excessive values are rejected. Omission preserves stored SEO for older narrative clients. Media and free JSON-LD remain separate workflows. Publication stores SEO and narrative with the same revision and atomic CMS activity receipt.

The public category content response adds optional normalized `seo` from published content. Category page metadata reads that response with current registry fallbacks. CMS page SEO takes precedence over legacy Category SeoMeta for published English page settings; no second editor or replicated SeoMeta write is introduced. The sitemap gateway reads published category policies in one batch, honoring noindex; its optional canonicalUrl is checked by the Web sitemap against the page URL, omitting noncanonical URLs. CMS failure makes the sitemap source unavailable rather than emitting potentially noindexed categories. Other locales keep the previous policy. The global launch-indexing switch remains an upper bound on category robots flags.

### Shared FAQ editing and category presentation controls — 10 September 2026

Admin/Content Manager `GET /admin/faqs?page=` lists 20 draft-aware records (page 1–100000) as entryKey/question/revision/status. `GET/PATCH /admin/content/faq-:uuid` uses the same content edit contract, actor authorization, operation UUID, optimistic revision and atomic CMS receipt as existing content resources. A new key GET returns `initial` without creating data. Its exact fields and limits are documented in the shared FAQ content slice; immutable identity and unknown fields are rejected. No delete/unpublish route is introduced.

Public `GET /content/faq` accepts locale, page (default 1), limit (1–100; default 100), category (one of the five FAQ topic keys), relatedCategory (one frozen family key), and contact=true/false. Filters combine with AND; results are ordered by sortOrder then entryKey and include entryKey/question/answer only, with standard total/page/limit metadata. English-only first slice; other valid locales return empty. The service always filters publication and never honors draft input. CMS failure is 503, not an empty list. The category consumer follows pagination, and retains its previous FAQ on an upstream error. Enabling useSharedFaq and selecting zero answers is a distinct successful empty state that hides the block.

Category PATCH additionally permits useSharedFaq/useEditorialApplications booleans, applicationsHeading/applicationsIntro and applicationNotes [{title, description}]. Missing fields preserve existing draft values; explicit arrays replace the list. Invalid row shapes, lengths, switches and unknown row keys are rejected. These fields publish with narrative/SEO in the existing CMS transaction.
