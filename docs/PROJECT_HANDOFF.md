...
**Authoritative snapshot: 29 August 2026 on `feature/design-system`.** The implementation and [ROADMAP.md](./ROADMAP.md) Current Status override the historical bootstrap snapshot retained below.
...
- Current catalog review state has no human decisions: no `TechnicalReview`, approved Specification or approved ProductClaim exists in live DEV. Four coolant Specifications are now `NEEDS_REVIEW`; the external sources remain uncaptured.
