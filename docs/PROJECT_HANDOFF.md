...
**Prepared:** 7 August 2026 · **Updated:** 30 August 2026 · **Snapshot:** `6b82485` on `feature/design-system` · **Phase:** Architecture frozen through ADR-018; `apps/api`, `apps/web` and `apps/cms` are all implemented applications. The authoritative current state is [§4 Current Repository State](#4-current-repository-state); bootstrap-era descriptions elsewhere in this document are retained as provenance and are labelled where they appear.
...
**Architecture is frozen and complete.** Every major area has been designed, reviewed, and approved: frontend, CMS content model, data model, i18n, SEO, security, RAG, and the full API contract. Eighteen ADRs record the contested decisions — ADR-001 through ADR-018, all Accepted ([ADR/README.md](./ADR/README.md)).

**Implementation is well past bootstrap.** The infrastructure layer (monorepo, workspace install, tooling, CI Phase 1, Docker development stack) exists and runs, the Prisma schema and its migrations are applied to `sam_platform`, and the frontend **design system foundation** is built in `packages/ui`/`packages/config`. On top of that, **all three applications are implemented**: `apps/api` (NestJS), `apps/web` (Next.js 15 — the public site plus the Admin Dashboard area) and `apps/cms` (Payload 3, isolated in `sam_cms`). What each one actually serves is stated once, in [§4 Current Repository State](#4-current-repository-state), and is deliberately not restated here.

> **Superseded bootstrap claim, retained as provenance — not current.** This overview previously read: _"`apps/web` and `apps/cms` are still `.gitkeep`-only. The next boundary is the frontend design proof (step A-3) — scaffolding `apps/web` and rendering the design system so it can be verified in a browser for the first time."_ That was accurate when the design-system foundation first landed. At `6b82485` it is not: `apps/web` tracks 314 files, including 25 route entries under `apps/web/src/app/`, and `apps/cms` tracks 34 files including `apps/cms/src/payload.config.ts`. A single leftover `apps/web/.gitkeep` is still tracked and is not evidence of a placeholder. The step A-3 boundary and the rest of that plan are recorded in [§5 Historical Implementation Order — superseded](#5-historical-implementation-order--superseded).
...
**37 markdown documents + 2 source spreadsheets** tracked under `docs/` at `6b82485`. The tables below name the core reading set rather than all 37. Read in roughly this order.
...
| [ADR/README.md](./ADR/README.md)               | Decision log — ADR-001 through ADR-018                                     |
...
- `apps/cms` is a Payload 3 application isolated in `sam_cms`. `apps/cms/src/payload.config.ts` registers the `Users`, `Pages` and `Media` collections and **four** company Globals — `AboutUs`, `CustomizedSolutions`, `QualityCertifications` and `ContactUs` — and Media/SEO foundations plus all four Globals are wired end to end through NestJS. `ContactUs` is served by `GET /content/globals/contact-us` and consumed by the Contact page in `apps/web`; the gate note at the top of this document states what it does and does not hold. The remaining content model is incomplete.
...
- ADR-001 through ADR-018 are accepted. The latest delivered boundary is **ADR-018**: named, bounded and transactional incremental catalog patches, without repair, approval or public-source leakage. ADR-017 — versioned subject-specific review hashes, atomic stale-approval invalidation and immutable source identity — remains the review-integrity boundary beneath it.
- Catalog review state **as recorded at the ADR-018 data gate (29 August 2026), carried forward and not re-verified in the 30 August 2026 documentation reconciliation** — no database, service or test was run in that Gate: there are no human decisions; no `TechnicalReview`, approved Specification or approved ProductClaim exists in live DEV. Four coolant Specifications are now `NEEDS_REVIEW`; the external sources remain uncaptured.
...
- **Latest catalog data gate — bounded incremental normalization (29 August 2026):** [ADR-018](./ADR/ADR-018-bounded-incremental-catalog-patches.md) adds a named-patch mechanism without broadening the ADR-015 importer into reconciliation. `coolant-source-layout-v1` (`b049c5e3f8c9f4e3698c7fc7fcbf3be03008b8cf41bd18fc8f447b68499e45f4`) passed unit guards plus disposable-clone apply/replay/rollback tests. A custom-format backup of pre-apply `sam_platform` was retained at `C:\Users\Hossein Salahiyan\AppData\Local\Temp\sam-platform-backups\sam_platform_pre_increment_20260829.dump` (SHA-256 `1ec8a0938ca8da22305c60cefd07d2d8251e3b8814e79dd60073680afe14d510`, 439,346 bytes) and restored successfully before apply. Live ImportRun `1790d306-1300-563a-9a54-d94fc16ca534` added 2 SpecProperties, updated 2 exact mappings, and inserted 4 Specifications plus 4 PRIMARY evidence links for `SAMCAT-W1-R294` / `SAMCAT-W1-R297`. Counts are now 28 SpecProperties, 1,402 Specifications, 1,402 SpecificationEvidence and unchanged 1,661 SourceFacts. The four new subjects preserve the printed values/methods, are `NEEDS_REVIEW`, have valid v2 hashes, and remain absent from `v_specification_public`; `technical_reviews` remains 0. A post-apply dry-run reports `ALREADY_APPLIED` and writes nothing.
- Admin proxy correction on 29 August 2026: the shared Nginx template now forwards `$http_host` as both `Host` and `X-Forwarded-Host`, preserving a development port such as `:8080`. The previous `$host` normalization made Next.js compare `Origin: localhost:8080` with `x-forwarded-host: localhost` and reject every legitimate login Server Action as an invalid request. This changes no route or authentication rule; it restores the same-origin information at the proxy boundary.
...
5. ~~Vercel/VPS split~~ — **resolved 7 August 2026.** The whole platform deploys to a single Linux VPS via Docker Compose behind Nginx; the Vercel split-hosting proposal is dropped. Reconciled across [TECH_STACK.md](./TECH_STACK.md), [ARCHITECTURE.md](./ARCHITECTURE.md), [DEVOPS.md](./DEVOPS.md), and [technology/FRONTEND_STACK.md](./technology/FRONTEND_STACK.md). The CORS concern is resolved with it — `web` and `api` now share one origin behind Nginx. Recorded as [ADR-005](./ADR/ADR-005-vps-docker-deployment.md). ~~Still to build: the Docker and GitHub Actions files themselves.~~ **Built, as of at least 18 September 2026** — three Dockerfiles under `docker/apps/`, `.github/workflows/ci.yml` and `.github/workflows/release-images.yml`; see item 6 below.
6. ~~The VPS does not exist, and its purchase is deferred until the application is complete.~~ — **resolved by at least 18 September 2026.** A VPS is provisioned, `samgp.com` resolves to it, and the platform has been deployed to it and verified live (most recently commit `84e9567`, confirmed via direct `curl` and a fresh PageSpeed Insights run). CI/CD Phase 2 (build and publish images) is implemented and runs automatically on merge to `main`; deployment itself is a manual step (`deploy/deploy.sh`), not an automated CI phase — see [DEVOPS.md § CI/CD](./DEVOPS.md#cicd-github-actions) and [§ Deployment Target](./DEVOPS.md#deployment-target) for the corrected detail. **One consequence carried forward:** the VPS's own copy of `docker-compose.yml`/`docker-compose.prod.yml`/`.env` is not a git checkout and is not synced from this repository by any tooling, so a compose-file change needs a separate manual step on the server — see the same DEVOPS.md section.
...
The bootstrap, frontend design proof, canonical public routes, Payload foundation, **four** CMS-backed company Globals (`AboutUs`, `CustomizedSolutions`, `QualityCertifications`, `ContactUs`), public lead writes, authentication/session lifecycle, lead Admin workflow, catalog import and the technical-review surface — including the Phase C decision controls described in §4 — are implemented. ADR-014 through **ADR-018** now govern the active catalog work.
...
Before starting it, read ADR-014 through ADR-018 and the relevant Admin/API contracts, identify every file involved, report the implementation plan, and obtain the separate code approval required by `CLAUDE.md`.
...
- Admin entry verification and Login UI pass on 29 August 2026: the supported access route remains `/login` → `/admin` → `/admin/catalog/review`; locale-prefixed Admin routes remain invalid by design. Live checks through Nginx at `http://localhost:8080` confirmed the Login document and API health return HTTP 200, an unauthenticated Technical Review request returns HTTP 307 to `/login`, and a Login Server Action POST is accepted without the former `Invalid Server Actions request` failure. Completing the authenticated leg still requires a valid authorized staff account; no credential, role, permission, cookie, middleware, endpoint or database behavior was changed. The Login page is now a responsive SAM-branded split entry screen using the official local mark, a truthful internal-workspace summary, reduced-motion-aware entrance treatment, one semantic H1 and the unchanged accessible email/password form. Verification: web TypeScript passes, all 54 web test files / 1,123 tests pass, root lint passes and `git diff --check` passes.
- Technical Review usability pass on 29 August 2026: the authenticated queue is now presented as a focused technical-control workbench rather than a flat report. A midnight summary identifies the matching queue, page-local `Needs review` and unresolved-finding counts, and current pagination without misrepresenting page counts as global totals. The status guide and five filter groups use native, keyboard-operable disclosure controls; active filters still open automatically, every filter remains a link and the query/API contract is unchanged. The table retains all eleven semantic columns and its named keyboard-scroll region, while adding a sticky header, finding-edge emphasis, row hover/focus feedback and a clear `Open review` action. Specification and Product Claim detail pages now pair Product context with Review status at wide widths and visually distinguish the permanent decision panel without changing panel order, evidence, decision inputs or Server Actions. Live QA through Nginx at 1267 px and a temporary 390×844 viewport found no page-level horizontal overflow, no console errors, a deliberately scrollable full-detail table, working disclosures, and a responsive 4→2-column operational summary; the viewport override was reset. A recoverable pre-pass branch exists at `codex/backup-technical-review-ui-2026-08-29` (`675f172`). Verification: web TypeScript passes, all 54 web test files / 1,124 tests pass, root lint passes and `git diff --check` passes.
- About Us dossier pass on 29 August 2026: a recoverable pre-pass branch exists at `codex/backup-about-us-2026-08-29`. The CMS-owned page now places a responsive, section-aware Company Dossier index between its Hero and supporting sections; it renders only links whose corresponding optional CMS section exists and keeps the structural anchors fixed across locales. Hero media is now requested eagerly with high fetch priority for better LCP, while all supporting imagery remains lazy. Anchor destinations account for the fixed header, hover/focus feedback is paired, and non-essential transitions respect reduced-motion preferences. The English editorial master was tightened around SAM Group's product-led B2B position without adding history, capacity, market, certification, customer or performance claims. No Payload schema, NestJS projection, public API, database model or technical-review boundary changed. The publisher file is updated but has not been executed in this pass, because that approved script republishes all three company Globals and may rotate the Payload service credential; a separate live-publication decision is required. Verification: all 54 web test files / 1,125 tests pass, web TypeScript passes and root lint passes.
- Contact Us completion pass on 29 August 2026: a recoverable pre-pass branch exists at `codex/backup-contact-us-2026-08-29`. The page now places a compact four-path enquiry navigator between its Hero and the shared Inquiry form, covering Product Information, Request a Quote, Sample Request and Export & Logistics. Every path sets one existing `InquiryType` query value and lands on the existing form; live checks confirmed all four values are preselected correctly. No endpoint, DTO, persistence or form submission behavior changed. The canonical Contact route now emits locale-aware canonical, Open Graph/Twitter metadata and a minimal claim-safe `ContactPage` JSON-LD node without inventing an address, telephone, opening hours or service area. Live output through Nginx returned HTTP 200, contained all four paths, carried no duplicate ids and emitted the expected metadata/schema. Verification: all 54 web test files / 1,125 tests pass, web TypeScript passes, root lint and `git diff --check` pass.

---

## 9. Documentation Reconciliation Gate — 30 August 2026

**Scope: documentation only.** No application code, test, package file, schema, migration, Payload collection or Global, API contract, or ADR decision was changed. Two files changed: this document and [ROADMAP.md](./ROADMAP.md). `origin/main` was not touched.

**Objective.** Bring the current-state and status portions of this document — and the one Current Status sentence in `ROADMAP.md` that had gone stale with it — into agreement with the implementation actually present at commit `6b82485`, while keeping superseded statements as labelled provenance rather than deleting them.

### Git preflight, verified before any edit

| Check                           | Result                                                                                     |
| ------------------------------- | ------------------------------------------------------------------------------------------ |
| Branch                          | `feature/design-system`                                                                    |
| `HEAD`                          | `6b824859e061ce1319a19038de54c69f24fe0682` — `feat(contact): add managed contact channels` |
| `origin/feature/design-system`  | identical to `HEAD`                                                                        |
| `origin/main`                   | `8fd74d7602e930a4fdb015a45001160d699d650d` — an **ancestor** of `HEAD`, not moved          |
| Distance                        | 102 commits ahead of `origin/main`, 0 behind                                               |
| Tracked worktree before editing | clean; untracked only `.agents/`, `.claude/`, `AGENTS.md`, none of them documentation      |

### Contradictions corrected, and the evidence for each

| Corrected claim                                                                                                                                                                    | Evidence at `6b82485`                                                                                                                                                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Snapshot SHA `73b3e36…` / 78 commits ahead → `6b82485…` / 102 ahead                                                                                                                | `git rev-parse HEAD`; `git rev-list --count origin/main..HEAD`                                                                                                                                                                          |
| "Six ADRs" and "ADR-001 through ADR-006" → ADR-001 through ADR-018                                                                                                                 | 18 files under `docs/ADR/`; [ADR/README.md](./ADR/README.md) lists all 18 as Accepted                                                                                                                                                   |
| "latest delivered boundary is ADR-017" → ADR-018                                                                                                                                   | [ADR-018](./ADR/ADR-018-bounded-incremental-catalog-patches.md); §4 already cited it as delivered — an internal contradiction                                                                                                           |
| Three CMS-backed company Globals → **four**, including `ContactUs` (§4, §8, ROADMAP)                                                                                               | `apps/cms/src/payload.config.ts` → `globals: [AboutUs, CustomizedSolutions, QualityCertifications, ContactUs]`; `apps/api/src/modules/content/contact-us.projection.ts`; `apps/web/src/features/contact/sections/contact-directory.tsx` |
| "`apps/web` and `apps/cms` are still `.gitkeep`-only" and "`apps/web` not yet scaffolded" → all three applications implemented; the old wording kept as a labelled superseded note | `apps/web` 314 tracked files with 25 route entries under `apps/web/src/app/`; `apps/cms` 34 tracked files including `payload.config.ts`; `apps/api` 275                                                                                 |
| "33 markdown documents" → 37                                                                                                                                                       | `git ls-files 'docs/**/*.md'`                                                                                                                                                                                                           |
| "Homepage/UI iteration … not committed at this snapshot" → committed                                                                                                               | `bb057f7` and its successors are ancestors of `6b82485`; tracked worktree clean                                                                                                                                                         |
| "read-only technical-review surface" (§8) → includes the Phase C decision controls                                                                                                 | §4 of this document; `apps/web/src/app/(admin)/admin/catalog/review/**`                                                                                                                                                                 |

### What this Gate deliberately did not do

- **No database, service, build, migration, seed, generate or test was run.** Every prior verification statement retained in this document keeps the date and scope it was recorded under, and the two that read as current — the live-DEV catalog review state in §4 and the suite counts in §8 — are now explicitly marked as carried forward rather than re-verified here.
- **No demo, imported, source-recorded, needs-review or unapproved commercial data was reclassified.** The DEMO / PLACEHOLDER Product and Blog rows, the 100-record product copy, the HSB source-recorded facts and the `NEEDS_REVIEW` coolant Specifications all keep their existing qualifiers. Nothing became "approved production content".
- **No frozen boundary was altered.** §6's frozen decisions, the ADR set, the ADR-002 database split, the ADR-003 single-API-surface rule and the ADR-010/011 slug namespace are unchanged in substance.
- **No historical record was deleted.** The bootstrap snapshot in §4, the implementation order in §5, and the superseded `73b3e36` reference are all retained; where a superseded statement sat above the historical marker, it was subordinated in place rather than removed.
...
## 2026-09-09 — ADR-024 implementation checkpoint (uncommitted)

User management and audit foundation is implemented in the working tree; this is not completion of the content editor. The dashboard and all AdminShell consumers now use AdminNav, including Users and Security events. Audit reads expose the affected user and support actor/event/outcome/inclusive UTC time filters with matching count/pagination. The user-management schema migrations were already applied locally; no production deployment exists.

Executed validation: API suite 1816 passed (307 opt-in tests skipped), Web suite 1754 passed; API types/build passed; Web production build passed after starting the previously stopped local API. Three PostgreSQL integration tests were separately enabled and passed on a schema-only disposable database: account creation/password verification, role/status and stale revision handling, persisted success events/disable revocation, rollback on audit failure, concurrent mutual Admin demotion and self-lockout prevention. That disposable database was removed afterward. No real staff account was changed. Authenticated nginx :8080 browser verified the Users page, shared menu, audit table and failure filter/empty state. Creation through the browser UI was not exercised.

Outstanding: real site-content writes through the approved Nest-to-Payload gateway, Content Manager editorial entry/access, atomic CMS activity with reconciliation for cross-service writes, publication controls, remaining catalogue models/media/SEO integration. Existing Payload read-only service credentials must retain their published-only boundary. Audit authentication events remain best-effort and do not yet attribute login/refresh actors; retention purge is documented but not implemented. Do not label ADR-024 complete or the preview Save controls functional.

## 2026-09-09 — Company-content editor in working tree

ADR-024 now has an English company-content editor at `/admin/content` for About Us, Customized Solutions, Quality & Certifications and Contact Us. Admin and Content Manager can enter it. Next server actions call guarded Nest `/admin/content/:key`; Nest uses the existing read-service API key plus a separate local `PAYLOAD_EDITOR_SECRET` to enter a bounded custom Payload endpoint. Public REST write/draft permissions remain unchanged. Internal `editorial-events` records commit with changes in a serializable sam_cms transaction; a durable receipt is checked after commit. Payload's generated types include this collection. Local Payload development schema push created it; production migration remains outstanding. The secret was generated into the ignored API/CMS environment files and never printed.

The form supports text, repeaters, groups, choices, boolean SEO fields and English SEO text. Upload fields are preserved but not exposed. Rich-text fields preserve stored content until edited; editing currently saves plain paragraphs (formatting editor still outstanding). Internal IDs and timestamps are excluded from the form. All remaining site pages and product/category editing are NOT connected by this four-global implementation.

Real authenticated browser through nginx :8080 saved and published a deliberate editorial update to Customized Solutions (hero title/support and introduction heading). Draft save left the public API's old title intact; publication returned success and both public API and nginx page then showed `Tell us what your application requires.`. This was a local content update, not deployment. Other global content was read, not overwritten. Full Web suite: 1754 passed; CMS suite: 137 passed; gateway tests: 4 passed; Web production build passed. CMS test runner requires escalation in this Windows sandbox because tsx fails on `uv_os_get_passwd`; the executed unrestricted run passed. Concurrency, rollback and CMS receipt tests still need fuller integration coverage before describing the editor as production-ready.

English Website/SEO masters were revised to remove unsupported finder-filter claims and internal publishing instructions. The 100-product draft copy file was regenerated; a comparison against HEAD verified product names, source identities, families/types and technical-review/publication-block flags unchanged. These product drafts have NOT been loaded or published. Base Oils/Bright Stock coverage and all remaining page content still need reconciliation with live catalogue.

### Content-editor reliability follow-up — 9 September 2026

The earlier sentence saying other Globals were only read is superseded for Quality & Certifications: a local draft of its hero was saved during a concurrent-write check (one successful response, one 503); it was not published. Customized Solutions remains the only page published in this editorial work. No additional content was written in this follow-up.

The company editor now checks the latest revision outside a rolled-back transaction, returning 409 when another edit won, while unconfirmed infrastructure failures remain 503. Six new mocked transaction scenarios cover stale revisions, reused operation IDs, audit-write failure/rollback, missing durable receipts, concurrent revision changes and successful saves. These are handler tests, not new database integration evidence. The updated conflict mapping has not yet been rechecked against the live CMS process. The web editor clears its browser reload/leave warning after a successful save; this does not guard client-side link navigation.

Executed against this working tree: CMS 144 tests passed; Web 1754 tests passed; CMS and Web TypeScript checks passed; scoped ESLint and Prettier checks passed; git diff whitespace check passed. The Windows sandbox prevented tsx startup with `uv_os_get_passwd returned ENOMEM`; the same CMS test command passed with escalation. No commit, push or deployment in this follow-up. Remaining production migration, full content coverage, rich-text formatting editor, product writes and unified editorial-log display are still outstanding.

## 2026-09-09 — Product editorial writes and content activity (working tree)

Implemented real `/admin/catalog/products` search/category/pagination and `/admin/catalog/products/:id` editor. Admin and Content Manager can edit English name, description and bounded text/robots/keywords SEO in one place. The dashboard and shared menu link to this real list, while the old design preview remains explicitly separate. Product drafts are in the new Catalog-owned `ProductEditorialDraft`, not in technical `ProductCopy`. Migration `20260909200000_product_editorial_drafts` was applied to local sam_platform, creating an empty additive table. Generated Prisma client refreshed. Publication writes Product and SeoMeta via SeoService with an audit record in the same serializable transaction. Slugs, category/type assignments, grades, specifications, claims, media and technical approval are not writable through this slice.

The existing `/admin/audit` page now selects account/security or content activity. The Admin-only Nest `/admin/content-events` route reads a protected CMS history endpoint using the independent editor credential. Projection excludes request hashes and internal receipt identifiers. Filtering and bounded pagination stay at the source database; CMS failure is an unavailable state, not empty history. Product draft/publication events are in platform activity. CMS receipts remain in sam_cms, with no replicated or falsely atomic cross-database write.

Executed: full API 1829 passed, 310 skipped; CMS 145 passed; full Web 1754 passed before adding three action tests, then focused 59 passed including all three new action tests. Three PostgreSQL integration tests separately passed on schema-only `sam_platform_disposable_editor_20260909`: draft/public isolation, atomic SEO publication/stale refusal, rollback on audit failure, concurrent single-winner saves. The disposable database was dropped; no real product was edited. API build/types and all three app types passed; repository-wide ESLint passed. Final name/object validation received an additional focused check. No push, commit or deployment.

Runtime blocker: automatic approval review rejected restarting the verified local API/web/CMS processes because the review service reported exhausted usage quota. The restart did not execute. The running preview therefore still predates the new product editor/history; final new-route browser checks and the updated Web production build remain outstanding. Do not bypass this rejection by starting replacement processes. The migration and tests above completed before this rejection.

Still unfinished: category/shared/public-page CMS coverage beyond the four company Globals, product applications/features/FAQ and media/document workflows, unpublication/new-product workflows, complete English editorial publication, richer text formatting, CMS production migration and security-log retention/attribution hardening. The whole ADR-024/content request is not complete.

Final continuation checks: full Web suite now 1757/1757 across 82 files, Web types pass, product DTO boundary tests 5/5 (including missing/nonnested content, arrays, whitespace-only names and unsafe URLs), whitespace and scoped formatting clean. Dashboard links now point to the real product list and company content; the design preview is no longer its primary entry. Runtime remains unchanged due to the rejected restart above.

### Runtime verification completed after renewed authorization

The prior approval-service quota blocker cleared. API and CMS were restarted and the Web production build passed. PowerShell `Stop-Process` initially failed with `Object reference not set to an instance of an object`, leaving ports occupied; the same verified process IDs were then stopped through the Windows process API, after which the updated services started on the original ports. No alternate deployment or proxy bypass was used.

Authenticated browser via nginx :8080 verified the real Products list with 102 records and six category options, the Base Oil Group I editor with existing description and SEO fields, and `/admin/audit?source=content` with the three real company-content receipts (Customized Solutions save/publish and Quality draft). This browser pass was read-only; product mutation/concurrency/rollback evidence remains the three separately executed disposable-database tests. No real product content was changed. The runbook in `docs/content/ADMIN_EDITOR_RUNBOOK.md` lists supported fields and remaining scope; environment examples now document the blank, fail-closed editor secret.

## 2026-09-10 — Product applications, features and FAQ

The product editor now supports bounded Applications/Features and FAQ rows. A nullable `publishedSections` snapshot on ProductEditorialDraft separates public output from ongoing edits. It is updated only by publication in the existing product/SEO/audit transaction. Legacy omissions preserve rows; explicit empty arrays remove them after publication. The English detail endpoint projects only published section fields; other locales and list DTOs are unchanged. Public rendering uses the existing Flagship criteria grid and native details/summary FAQ controls, with empty sections omitted and text escaped. No real product rows/content were seeded or changed.

Migration `20260910090000_product_editorial_sections` applied locally. Full API 1831 passed (311 opt-in skipped); full Web 1760 passed. Four real PostgreSQL tests separately passed on schema-only disposable database `sam_platform_disposable_sections_20260910`, including draft isolation, section removal, legacy omission preservation, rollback and concurrency. The disposable database was removed. API build and both app TypeScript checks passed. This advances the product editor only; remaining category/shared content and media workflows are still open.

Web production build passed and the original local API/web ports were restarted. Authenticated nginx browser verification of the Base Oil Group I editor showed all three new repeaters and the Save draft/Publish controls. That browser check was read-only; mutation evidence is from the disposable database, not a real product write. Public nonempty section rendering and native FAQ semantics were verified by server-render tests; a populated mobile browser layout was not exercised in this gate. No deployment or new imagery.

## 2026-09-10 — Category narrative editor

Added the Payload-owned ProductCategoryContent collection and six category links under `/admin/content`. This bounded first slice edits hero, overview, quality introduction, packaging/supply narrative and documentation narrative. The initial editor is populated from the existing page registry without creating a CMS row. Category-prefixed resources reuse the existing authenticated editor's serializable transaction, revision comparison, operation receipt and audit write. Immutable category keys are unique; technical/classification, FAQ, media and SEO sources are unchanged.

Public `GET /content/product-categories/:key` projects only published English narrative. The category resolver overlays only ordinary strings and preserves existing page copy on absence/unavailability. No real category content was created or published in verification. Authenticated nginx :8080 browser showed the twelve populated Base Oils fields; public Lubricant Additives returned 200 and the category content API returned available:false. CMS dev schema push added the local collection/version tables; a production CMS migration baseline remains outstanding, as before. No deployment.

Validation: CMS 151/151, Web 1763/1763, focused API 12/12; all three app type checks, API build, Web production build and scoped lint passed. Category creation/update draft/publish transaction wiring uses mocks, alongside existing rollback/conflict tests; this gate did not execute real category mutations or a new mobile visual pass. Applications/industries, shared FAQ, category SEO and media remain subsequent slices; this is not complete category content coverage.

## 2026-09-10 — Category SEO editor

Category page ordinary SEO now lives in the same category content editor and Payload draft/publication transaction. It reuses the shared SEO field definitions for titles/descriptions, canonical, Twitter card, robots and keywords, with a bounded nested allowlist at the editor boundary. No second SeoMeta editor or replicated database write. Images and free JSON-LD remain deferred. English published page SEO overrides legacy entity policy in the sitemap; a single Content service batch supplies those policies. Sitemap canonical overrides travel to Web for same-page comparison against its configured origin. Noindex/noncanonical entries are omitted; CMS failure is unavailable rather than an incorrect inclusion set.

Web metadata and narrative share a React request-cached content read. Registry metadata stays the initial fallback, the environment indexing gate remains above per-page settings, and CMS failure renders existing content with noindex/nofollow. The local CMS development schema was updated; production migration baseline remains owed. No real category draft or publication was created for testing, and no deployment took place.

Validation: CMS 154 passed; focused API 26 passed; Web full suite 1770/1770 passed. All three type checks, API build, Web production build and scoped lint passed. Authenticated nginx browser showed the SEO section in the Industrial Oils editor with current title/description, canonical, sharing fields, robots checkboxes and keywords. HTTP output retained the existing title/canonical and noindex/nofollow; sitemap API returned all six existing English categories. First CMS cold request exceeded the API timeout while compiling; a subsequent request returned normally. Mutation policy tests are fixture-based, not real content publication; no new mobile visual pass.

## 2026-09-10 — Shared FAQ library and category applications

Admin and ContentManager can list/create/edit versioned FAQ answers at `/admin/faqs` through Nest and the existing authenticated Payload editor. New reads create no rows. Creation/update, draft/publication and receipts reuse the serializable transaction and revision protections. FAQ entries use immutable UUID keys, bounded plain paragraphs, five topics, optional multi-family soft references, Contact eligibility and display order. Public `/content/faq` provides paginated published English projections only.

Category editors now include explicit shared-FAQ and editorial-applications switches. Both default off, preserving previous fixtures. Successful empty published results hide the opted-in section; FAQ service failure preserves the existing answers. Applications accept bounded heading/intro and up to 20 title/description rows. Publication invalidates category output. Contact and standalone public FAQ consumers, photos/uploads and complete editorial population remain unfinished; this is not full site content coverage.

Validation: all three app type checks, API build, Web production build and lint passed; Web full suite 1774/1774, API content suite 202/202, CMS full suite 157/157 before adding four FAQ transaction cases (all four subsequently passed). Final category-focused Web suite 87/87. Authenticated browser through nginx :8080 showed the empty FAQ library, new question form and category switches/repeater/SEO. Public filtered FAQ returned 200 empty, invalid limit 400, unauthenticated Admin 401, and category fallback remained unchanged. A first cold CMS compile exceeded the API timeout (503); warmed checks passed. CMS dev schema was updated locally. Transaction evidence uses mocks; no actual FAQ/category draft or publication was created and no populated mobile browser pass was performed. Production CMS migration baseline remains required; no deployment.

## 2026-09-10 — Public FAQ and Contact integration

Added `/en/faq` with five topic filters, native accessible disclosures, Flagship typography/surfaces, enquiry CTA and footer entry. Shared public reader now serves category and Contact consumers; empty or unavailable Contact FAQ omits the section without blocking the enquiry form. FAQ publication revalidates all consumers and the sitemap. Only nonempty published default-locale FAQ enters the sitemap; filtered/empty/non-English views remain noindex and the site indexing gate still applies. FAQPage JSON-LD uses the same visible answers through the shared safe serializer. The page heading/metadata remain code-owned; a dedicated page-level SEO/content editor is still outstanding.

Seven English answers in `docs/content/site-copy/ENGLISH_SHARED_FAQ.json` were published to the LOCAL CMS through the authenticated audited editor with durable receipts, using stable entry UUIDs and preserving existing records. Four are Contact-eligible. They explain enquiry preparation without invented response times, MOQs, prices or certifications. No category FAQ switch, product technical data or production service was changed.

Validation: Web 1777/1777 tests (88 files), lint, type checking via production build, build and diff check passed. Browser at nginx :8080 showed the seven questions and topic navigation; HTTP verification confirmed export topic one answer, Contact four answers and the FAQ sitemap entry, all 200. Mobile CSS uses the shared type scale with horizontally scrollable topic controls; no screenshot-based mobile geometry audit was performed. The prior footer test incorrectly treated `/fa/faq` as a repeated locale prefix; it now checks the complete path segment. No deployment. Photos and richer page-level editorial coverage remain outstanding.

## 2026-09-11 — FAQ page content and SEO editor

`/admin/content/faq-page` now manages seven page text fields and the shared text/robots/canonical/social SEO subset in one audited form, available to Admin and ContentManager. The separate FAQ library remains the only answer store. Public `GET /content/globals/faq-page` projects published complete English fields only. Metadata and page text share a request-cached read; noindex and external canonical policies are reflected in the sitemap, with the launch indexing gate retained. The existing public text was initialized in the LOCAL CMS without changing its wording.

Live verification exercised save-draft (public API stayed unavailable), stale revision rejection (409), then publication using the returned revision; both successful writes had durable audit receipts. Authenticated browser through nginx showed populated page text/SEO controls. Public text, canonical and noindex launch metadata were verified. CMS development schema updated locally; production CMS migration baseline remains required. No deployment.

Validation: CMS 164/164, Web 1779/1779, content API 204/204; type checks, API/Web builds, scoped lint and diff checks passed. Mobile geometry was not remeasured in this form-only update; it uses the existing content form. FAQ page topic vocabulary/order remains code-owned. Social image upload and arbitrary structured-data override editing remain part of the deferred media/advanced SEO work.

### 13 September 2026 — structural editorial integration

Home, Products landing, Export & Logistics, Header and Footer are wired to versioned Payload Globals through the existing Nest editor and public reader. The Admin content index links to all five. Existing layout, image assets, canonical families and routes remain unchanged. Text groups, bounded repeaters and page SEO use the existing draft/publish/revision/audit workflow; header/footer do not duplicate SEO.

Validation: Web 1,792/1,792; CMS 177/177; final focused API 22/22 (32/32 in the earlier broader pass). Web/CMS type checks, API build and repository lint passed. The later Web build compiled, validated types and generated 52 pages, but standalone packaging failed on Windows symlink EPERM with the separate uncommitted deployment configuration. This is not a successful full production build. Runtime verification used normal next dev through nginx.

Live nginx :8080 verification: all five Globals saved and published existing English copy in the LOCAL CMS, with draft isolation and durable save/publish audit receipts verified. A stale second Header tab was rejected. A temporary Header CTA remained private as a draft, appeared in the public mobile menu after publication, and was restored to Request a Quote. The encoded Find &amp; download label was corrected. Public reads for all five return 200/available:true. Browser checks covered Products, Export and Home at the actual 603px viewport without horizontal overflow; requested 390/1280 overrides were not applied, so those exact widths are not certified.

Initial unsaved Payload arrays receive approved defaults; intentionally saved empty arrays remain empty. Navigation destinations and document access classifications remain code-owned. Docker recovery completed with owner authorization using stale runtime socket directory backups, without resetting database volumes. CMS local development schema push completed, including removal of two task-created decorative spacing columns. Production still requires an additive Payload migration for the five Globals. No deployment. Separate deployment changes remain outside this content commit, including the migrate script, API prisma dependency, standalone Web output and deployment files.

### 14 September 2026 — Linux deployment package verified locally

API, CMS and Web Docker images now build and run locally. Fixed missing workspace manifests/sources, TypeScript runtime exports and build-workspace links in packaged dependencies. The CMS CLI retains its TypeScript tooling and loads its configuration without a database connection. Docker context excludes nested environment files and host-generated artifacts. API health/locales/published Header reads and CMS Admin/published Header reads returned 200; packaged Web Home/Products/Login returned 200 and unauthenticated Admin redirected to Login. All 337 API and 670 CMS CLI dependency versions matched the lockfile. See deploy/PACKAGE_STATUS.md for verification limits and earlier failures.

Owner freed C drive space and cancelled Docker relocation before any data moved. Temporary test containers and credential files were removed; original development containers and volumes remain. No server deployment, registry publication or database migration was performed. Draft deployment/backup scripts and the untracked CMS migration baseline remain outside this package commit. Production migration completion, full-stack setup, TLS, object storage and backup/restore verification remain owed.
