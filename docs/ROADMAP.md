...
**Next: implement and verify the accepted [ADR-011](./ADR/ADR-011-products-slug-namespace-enforcement.md) database enforcement, before the first `Product` write.** The mechanism is no longer an open architecture question — ADR-011 closes [ADR-010](./ADR/ADR-010-products-slug-namespace-and-collision-policy.md) §6 and freezes a shared, database-maintained `ProductSlugClaim` registry whose normalized `slugKey` unique key is the authority, with statement-level triggers, release-before-claim, and no advisory locks. **None of it is built**: no table, no index, no trigger, no migration, and no change to `prisma/schema.prisma`. The gate is one Database task — the atomic installation ADR-011 §11 specifies, with a generic backfill covering every existing `Category` and `Product` base slug and every `Category`/`Product` slug translation, followed by executed verification. ADR-010 §6 still requires that enforcement to be in place **before** the first `Product` write, the first Product reference data, and the first translated `Category` or `Product` slug row, so **nothing that writes into the `products/` namespace may proceed ahead of it** — this roadmap does not authorize Product writes.

Explicitly **not** the next gate: frontend scaffolding and Category seeding are both complete.

**Not yet implemented or deferred:** no `Product` rows or Product reference data; no durable slug-collision enforcement (the mechanism is decided in ADR-011, and none of it is built — no `product_slug_claims` table, no trigger, no migration); no Product Detail frontend branch (the shared namespace segment exists, the branch does not); no Product Finder backed by a real API; no `ProductType` vocabulary or reference data, and therefore no `SegmentProductType` or `ProductSegment` membership rows; `apps/cms` holds only `.gitkeep`, so no Payload, no Content module fronting it, and no CMS-owned editorial content; no blog; no inquiry or form submission endpoints; no authentication or RBAC; no Admin Dashboard; no `ContentTranslation` or `SeoMeta` rows for any Category; no application Dockerfiles and no CI Phases 2–3; no production deployment or hardening. Root `README.md` is still empty.
