...
What Payload *does* own for these areas is the **editorial wrapper content** around that structured data — exactly the same "one capability, two storages, unified by NestJS" pattern already used twice (SEO: [SEO_ARCHITECTURE.md §0](../seo/SEO_ARCHITECTURE.md#0-the-core-design-problem-and-how-its-resolved); i18n: [INTERNATIONALIZATION_STRATEGY.md §0](../i18n/INTERNATIONALIZATION_STRATEGY.md#0-the-core-design-problem-and-how-its-resolved)). This is the third application of that pattern, not a new one.
...
| **Who manages** | Content Manager, Admin |
| **Localization** | Fully localized |
| **Fields** | `headingTitle`/`introText`; `whatCanWeCustomize` (**array**: `title`, `description` — 5 entries today); `customizationProcess` (**array**: `stepNumber`, `title`, `description` — feeds the GSAP-driven `CustomizationProcess` component per [FRONTEND_ARCHITECTURE.md §8](../frontend/FRONTEND_ARCHITECTURE.md#8-animation-architecture-framer-motion--gsap)); `privateLabelProgramme` (rich text + `weProvide`/`youProvide` **arrays**); `caseExamples` (**array**: `title`, `requirement`, `solution` — anonymized, currently placeholders per [SITE_STRUCTURE.md](../SITE_STRUCTURE.md#5-customized-solutions)) |
...
| **Localization** | Fully localized |
| **Fields** | `heroTitle`; `qualityApproach` (**array**: `stage` [Incoming/In-Process/Outgoing], `description`); `laboratoryCapability` (rich text + `testsPerformed` **array**); `certifications` — **relation to the `Certifications` collection** (§4), not an embedded array, since certificates are referenced from more than one place (see §4); `documentationProvided` (**array**); `samplingPolicyText` (rich text) |
| **Rich text** | Lab capability intro, sampling policy |
| **Media** | Hero image; lab/testing photography (blocked on the outstanding shoot per [SITE_STRUCTURE.md](../SITE_STRUCTURE.md#outstanding-confirmations-needed)) |
| **Relationships** | **→ `Certifications` collection** (many, via relation field) |
...
| **Who manages** | Content Manager, Admin |
| **Localization** | Fully localized (contact *copy*; raw addresses/phone numbers are locale-independent facts — see Fields) |
| **Fields** | `heroTitle`; `contactOptions` (**array**: `title`, `description`, `ctaLink` — 4 cards); `contactInformation` (**group, not localized**: `headOfficeAddress`, `factoryAddress`, `emails` [general/sales/technical], `phone`, `whatsappNumber`, `workingHours`, `mapEmbedUrl`); `globalInquiriesText` (rich text); `contactFaq` — **relation to `FaqEntries`** filtered by a "Contact" tag (§4), not a duplicate embedded list |
...
### ProductCategoryContent — *Collection*
...
The **Speculative Application** (CV upload, no listed role) and the **Distributor Application** form on the same page are Prisma submissions (future entities, not modeled here) — this collection is job *listings* only, never applications. Since listings are maintained by hand, an editor closing a role is a `status` change or a delete in Payload, not an automated sync — worth stating because it means listing hygiene is an editorial responsibility, with no external system to fall back on.

### FaqEntries — *Collection*
...
Two items previously listed here were closed without needing a decision: *Downloads' company-wide assets* (settled by decision 4's media boundary — any future gated-download lead tracking is a Prisma data-model question, tracked with the other Prisma gaps in [SITE_STRUCTURE.md](../SITE_STRUCTURE.md#data-model-gaps-surfaced-by-this-structure)), and the *`SEO_ARCHITECTURE.md` follow-up edit*, which has since been applied.
