...
### ADR-024 category narrative slice — 10 September 2026

`ProductCategoryContent` begins with the immutable, unique six-family `categoryKey` and bounded English narrative fields: heroTitle (300), heroSupportingText (2000), overviewHeading (300), overviewText (10000, plain paragraphs), qualityHeading (300), qualityIntro (5000), supplyHeading (300), packagingSupplyText (5000), supplyTerms (5000), documentationHeading (300), documentationIntro (5000), documentationNote (5000). All fields require non-whitespace text at publication. This explicit plain-paragraph first slice precedes the richer overview model above. Payload owns drafts/versions and atomic editorial receipts. No initial public content is replaced or auto-published. Existing code content fills the editor before its first save and remains the public fallback until publication.

Taxonomy, grades, methods, process claims, linked documents, images, shared FAQ relations and category SEO are not editable in this slice. FAQ remains owed to the shared FaqEntries model; no category-local duplicate is introduced. Full SeoFields, applications/industries and the remainder of the specification above remain outstanding. Category identity cannot be created, renamed or deleted through this editor.

### Category SEO implementation slice — 10 September 2026

ProductCategoryContent adopts the ordinary text, card-type, robots and keywords subset of the shared seoFields() group. The category page SEO owner is Payload per SEO_ARCHITECTURE §3; legacy Prisma Category SeoMeta is not a second editing surface. All edits publish atomically with the category narrative. Public metadata and sitemap policy consume published SEO only. Media upload relationships and structured-data overrides remain deferred. English-only rollout and existing content fallback are unchanged.

### Shared FAQ and category applications — 10 September 2026

`FaqEntries` is introduced as one independently versioned record per answer. First-slice fields: immutable `entryKey` UUID; question (1–300), answer (1–8000, plain paragraphs), topic (the five approved FAQ categories), relatedCategoryKeys (0–6 unique frozen family keys), showOnContactPage boolean and sortOrder integer 0–10000. The first slice explicitly uses plain paragraphs instead of rich text. Targeting uses the existing immutable category soft keys rather than CMS row IDs: an answer may be shared by several families without copying it or requiring unpublished category stubs. This refines the earlier singular relatedCategory relation; category identity remains Catalog-owned. REST mutation stays disabled and Admin/Content Manager editing uses the gateway's revision/audit transaction.

Category fields `useSharedFaq` (default false) and `useEditorialApplications` (default false) are explicit publication switches. Until enabled, existing category copy remains authoritative. Shared FAQ selection reads only published answers matching that family; an enabled empty set intentionally hides the FAQ block. The FAQ endpoint also supports the approved topic and contact filters for subsequent public-page consumers; the dedicated FAQ page/Global and Contact FAQ rendering remain separate slices, not implied complete here.

Custom applications comprise heading (300), introduction (3000), and up to 20 title/description rows (200/2000). Enabling the switch replaces the category's existing applications presentation; an empty published list intentionally hides that section. Taxonomy, downstream membership, grades, specifications and method approval are never mutated by this editorial content. Older editor requests omitting the new fields preserve their stored values.

### FAQ page editorial and SEO — 11 September 2026

The thin `faq-page` Global now owns seven bounded English text fields: eyebrow (150), title (300), introduction (3000), questionsHeading (300), contactHeading (300), contactText (3000), contactLabel (150). All are required, localized plain text. It shares the validated category SEO subset (search/social text, HTTPS canonical, indexing/following flags, card type and keywords); uploads and arbitrary structured-data overrides remain outside this slice. Answers continue to live only in FaqEntries. Topic vocabulary/order and CTA route stay code-owned in this implementation; the previously described editorial topic ordering remains outstanding.

Draft/version support and the authenticated editor revision/receipt transaction apply. Direct Global updates are disabled to preserve the audit boundary. Initial editor text uses the current page defaults without creating a row. Publication controls visible page text and metadata together; successful absence preserves existing text/SEO, service failure preserves text but fails closed for indexing/sitemap.

### Structural editorial integration — 12 September 2026

The existing bespoke Home, Products landing, Export & Logistics, Header and Footer layouts now read five versioned Globals through NestJS. Each global has explicit section text groups and bounded editorial array fields. The layout is still code; these are not generic page-builder blocks. Drafts, publication, revision conflicts and durable audit receipts use the existing company editor. Admin and ContentManager edit at /admin/content/{key}; Payload direct writes remain disabled for these globals.

English is the current editorial scope. Public absence retains the approved code fallback; infrastructure failures do not become published content. Page SEO shares the existing text/canonical/robots contract; Header and Footer have no second SEO record. Existing image assets and canonical family identities are unchanged. Array source keys retain the existing illustration and route identity while allowing text edits, ordering and removal; arbitrary new routes or family names are not introduced. Product catalogue data remains Prisma-owned.

This gate does not publish new company claims, translate FA/AR, acquire images or deploy. Production needs an additive Payload migration for these five Globals before serving this version; the separate deployment baseline must not be silently replaced.
