...
- **Catalog** — products, categories (the Product Family axis), segments, product types, specifications. Segments and product types are accepted architecture per [ADR-007](./ADR/ADR-007-product-taxonomy-v2.md) and their **Prisma schema is implemented** (migration `20260812160853_add_product_taxonomy_v2`), with the Product Detail response already serving a product's Segments and Product Type. The `GET /products` taxonomy filters frozen by [ADR-008](./ADR/ADR-008-b2-filter-contract-and-segment-vocabulary.md) are **approved but not implemented**. Segment reference data has its own implemented mechanism — the dedicated catalog seed `prisma/seed-catalog.ts` (`pnpm seed:catalog`), which populated the eight approved Segment rows in the local DEV `sam_platform`; no Product Type row or membership exists. The module boundary itself is unchanged
