...
| Product taxonomy    | **Product Family is single-valued** per Product; **Segment** is first-class and M:N with Product; **Product Type** is first-class and shared across Segments, never duplicated per Segment; one primary Product Type initially. **Schema implemented** (models `Segment`/`ProductType`/`ProductSegment`/`SegmentProductType` + nullable `Product.productTypeId`, migration `20260812160853_add_product_taxonomy_v2`); **Product Detail serves `segments[]` + `productType`**. **Catalog seed implemented** (`prisma/seed-catalog.ts`, run explicitly as `pnpm seed:catalog`); the eight approved Segment rows are populated in local DEV `sam_platform`. **Approved — not implemented:** the `GET /products?segment=&productType=` filters. No ProductType row or membership exists. | [ADR-007](./docs/ADR/ADR-007-product-taxonomy-v2.md) · [ADR-008](./docs/ADR/ADR-008-b2-filter-contract-and-segment-vocabulary.md)                                                                                                        |
| Product URLs        | **Canonical Product Detail URL is `/{locale}/products/{product-slug}`** — flat. Segment and Product Type are navigation/facet relationships, **never canonical URL ancestry**. The URL decision is frozen; **the frontend route is not implemented** — no product detail route or shared template exists in `web`. The backend half is live: `GET /products/:slug` already serves the taxonomy fields.                                                                                                                                                                                                                                                                                                                                                                               | [ADR-007](./docs/ADR/ADR-007-product-taxonomy-v2.md)                                                                                                                                                                                     |
...
| [`docs/ADR/README.md`](./docs/ADR/README.md)                                                                                                                                                    | Decision log — ADR-001 through ADR-008                    |
